...
import { RefreshCw, TrendingUp, TrendingDown, Minus, BarChart3, Activity, Settings2, RotateCcw, Database, Table2, ChevronDown, ChevronUp } from 'lucide-react'
import {
  Table,
  TableBody,
  TableCell,
...
const formatTime = (dtString) => {
  ...
  // Parse ISO string: "2026-04-11T19:31:38.421998+00:00"
...
export default function HistoryRegression({ datasetName }) {
  ...
  const fetchHistoryData = async () => {
    ...
    try {
      ...
      if (jsonData.length > 0) {
        ...
        setFeatureNames(Object.keys(firstItem.features || {}))
      }
  ...
  const timestamps = useMemo(() => {
    return [...data].reverse().map(d => formatTime(d.timestamp))
  }, [data])

  const actualValues = useMemo(() => {
    return [...data].reverse().map(d => d.actual)
  }, [data])
  ...
  const modelMetrics = useMemo(() => {
    ...
    modelNames.forEach(model => {
      const predValues = data.map(d => d.results?.[model]).filter(v => v !== undefined)
      metrics[model] = {
  ...
  const allPredValues = modelNames.flatMap(m => data.map(d => d.results?.[m]).filter(v => v !== undefined))
  ...
  const chartSeries = [
    {
      name: 'Actual',
      type: 'area',
      data: [...data].reverse().map(d => d.actual),
    },
    ...modelNames.map(model => ({
      name: model.toUpperCase(),
      type: 'line',
      data: [...data].reverse().map(d => d.results?.[model] ?? null),
      yAxisIndex: 1,
    }))
  ]
            ...
            {data.length === 0 && !error && loading ? (
              <div className="h-96 flex flex-col items-center justify-center text-muted-foreground">
                <div className="relative">
                  ...
                  <Database className="h-6 w-6 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-indigo-500" />
                </div>
              ...
              <div className="h-96 flex flex-col items-center justify-center text-muted-foreground">
                <div className="relative">
                  <div className="h-16 w-16 rounded-full border-4 border-amber-500/30 flex items-center justify-center">
                    <Activity className="h-8 w-8 text-amber-500" />
                  </div>
                </div>
                <p className="mt-4 text-lg font-medium text-amber-600">Dataset is idle</p>
                <p className="text-sm opacity-60">Please start worker</p>
                <p className="text-xs text-muted-foreground mt-2">Dataset: {datasetName}</p>
              </div>
            ) : data.length > 0 && !error ? (
              <>
                      ...
                      <Table>
                        <TableHeader className="sticky top-0 bg-white/90 dark:bg-gray-950/90 backdrop-blur-sm z-10">
                          <TableRow className="border-gray-200/50 dark:border-gray-800/50 hover:bg-transparent">
                            ...
                            {featureNames.map(featureName => (
                              <TableHead key={featureName} className="font-semibold text-xs text-right hidden lg:table-cell">
                        ...
                        <TableBody>
                          {data.map((row, idx) => (
                            <TableRow
                              key={idx}
                              className="border-gray-200/50 dark:border-gray-800/50 hover:bg-indigo-50/30 dark:hover:bg-indigo-950/20 transition-colors"
                            >
                              ...
                              {featureNames.map(featureName => (
                                <TableCell key={featureName} className="text-xs text-right font-mono tabular-nums hidden lg:table-cell text-muted-foreground">
